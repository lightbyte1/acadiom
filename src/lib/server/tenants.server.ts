...
/**
 * Server-side tenant functions
 * ALL database queries for server-side go here
 */

const ALLOWED_ROLES = ["owner", "admin", "teacher", "assistant"];

export async function getTenantsServer(userId: string): Promise<Tenant[]> {
...
export async function getTenantsByOrganizationIdServer(
...
export async function getTenantByIdServer(
  tenantId: string
): Promise<Tenant | null> {
  const supabase = await createServerClient();
  const { data, error } = await supabase
